...
class RiskManager(BaseAgent):
    def __init__(self, redis_publisher, llm_config=None, risk_config=None):
    ...
    async def _validate_proposal(self, data: dict) -> Signal | None:
        ...
        vix_regime_check = self._check_vix_regime()
        checks.append(vix_regime_check)
        if not vix_regime_check["passed"]:
            approved = False
        elif vix_regime_check.get("scalps_only") and trade_type != "SCALP":
            checks.append({
                "name": "vix_regime_scalps_only",
                "passed": False,
                "detail": f"VIX regime HIGH — only SCALP allowed, got {trade_type}",
            })
            approved = False

        if self._drawdown_mgr.should_pause_trading():
            drawdown_status = self._drawdown_mgr.get_status()
            checks.append({
                "name": "drawdown_circuit_breaker",
                "passed": False,
                "detail": f"Drawdown > 15%: {drawdown_status['drawdown_pct']}%",
            })
            approved = False
        ...
        # Apply drawdown manager size multiplier
        size_multiplier = self._drawdown_mgr.size_multiplier
        approved_quantity = int(approved_quantity * size_multiplier)

        # Apply VIX regime reduction (ELEVATED = 25% reduction)
        if vix_regime_check.get("regime") == "ELEVATED":
            approved_quantity = int(approved_quantity * 0.75)

        # Round down to nearest lot size
        if approved_quantity > 0:
            approved_quantity = (approved_quantity // lot_size) * lot_size
    ...
    def _check_vix_regime(self) -> dict:
        ...
        if vix > 28:
            return {
                "name": "vix_regime", "passed": False,
                "detail": f"VIX {vix:.1f} > 28 — EXTREME, trading paused", "regime": "EXTREME",
            }
        elif vix > 22:
            return {
                "name": "vix_regime", "passed": True,
                "detail": f"VIX {vix:.1f} — HIGH, scalps only", "regime": "HIGH", "scalps_only": True,
            }
        elif vix > 18:
            return {
                "name": "vix_regime", "passed": True,
                "detail": f"VIX {vix:.1f} — ELEVATED, reduce 25%", "regime": "ELEVATED",
            }
        else:
            return {
                "name": "vix_regime", "passed": True,
                "detail": f"VIX {vix:.1f} — NORMAL", "regime": "NORMAL",
            }
