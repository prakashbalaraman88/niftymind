...
class KiteExecutor:
    ...
    async def _place_regular_order_with_sl(self, trading_symbol: str, transaction_type: str,
        ...
        # Use smart order routing for entry instead of plain market order
        smart_result = await self._place_smart_order(
            trading_symbol=trading_symbol,
            transaction_type=transaction_type,
            quantity=quantity,
            product=product,
        ...
        entry_order_id = smart_result["order_id"]
        fill_price = smart_result["price"]
        if smart_result.get("slippage", 0) > 0:
            logger.info(f"Smart order entry slippage for {trading_symbol}: ₹{smart_result['slippage']:,.2f}"
    ...
    def _get_mid_price(self, trading_symbol: str) -> float:
        """Get mid-price (average of bid/ask) for a symbol from Kite quote data."""
        try:
            quote = self._kite.quote([f"NFO:{trading_symbol}"])
            data = quote.get(f"NFO:{trading_symbol}", {})
            depth = data.get("depth", {})
            best_bid = depth.get("buy", [{}])[0].get("price", 0)
            best_ask = depth.get("sell", [{}])[0].get("price", 0)
            if best_bid > 0 and best_ask > 0:
                return round((best_bid + best_ask) / 2, 2)
            # Fallback to LTP if depth unavailable
            return float(data.get("last_price", 0))
        except Exception as e:
            logger.warning(f"Failed to get mid-price for {trading_symbol}: {e}")
            return 0

    async def _place_smart_order(self, trading_symbol: str, transaction_type: str,
                                  quantity: int, product: str = "MIS",
                                  exchange: str = "NFO") -> dict:
        """Place limit order at mid-price, widen by Rs.1 each attempt (3x), then market fallback.

        Returns dict with keys: filled, price, slippage, order_id, fallback_market.
        """
        mid_price = await asyncio.to_thread(self._get_mid_price, trading_symbol)
        if mid_price <= 0:
            logger.warning(f"Smart order: no mid-price for {trading_symbol}, falling back to market")
            order_id = await asyncio.to_thread(
                self._kite.place_order,
                variety="regular",
                exchange=exchange,
            ...
            fill_price = await self._poll_until_filled(order_id, "regular")
            return {"filled": True, "price": fill_price, "slippage": 0, "order_id": str(order_id), "fallback_market": True}

        for attempt in range(3):
            if transaction_type == "BUY":
                price = round(mid_price + (attempt * 1.0), 2)
            else:
                price = round(mid_price - (attempt * 1.0), 2)
            ...
            try:
                ...
                try:
                    ...
                    if status == "COMPLETE":
                        fill_price = float(last_update.get("average_price", price))
                        slippage = abs(fill_price - mid_price)
                        logger.info(
                            f"Smart order filled: {trading_symbol} {transaction_type} x{quantity} "
                            f"@ ₹{fill_price:,.2f} (mid=₹{mid_price:,.2f}, slippage=₹{slippage:,.2f}, attempt={attempt+1})"
                        )
                        return {"filled": True, "price": fill_price, "slippage": round(slippage, 2),
                                "order_id": order_id, "fallback_market": False}
                except Exception as e:
                    logger.warning(f"Smart order: failed to check order {order_id}: {e}")
                ...
                try:
                    ...
                    logger.warning(f"Smart order: failed to cancel order {order_id}: {e}")

            except Exception as e:
                logger.warning(f"Smart order attempt {attempt+1} failed for {trading_symbol}: {e}")

        # Fallback: market order
        logger.info(f"Smart order: 3 limit attempts exhausted for {trading_symbol}, placing market order")
        order_id = await asyncio.to_thread(
            self._kite.place_order,
            variety="regular",
            exchange=exchange,
        ...
        order_id = str(order_id)
        fill_price = await self._poll_until_filled(order_id, "regular")
        slippage = abs(fill_price - mid_price) if fill_price > 0 else 0
        logger.info(
            f"Smart order market fallback filled: {trading_symbol} {transaction_type} x{quantity} "
            f"@ ₹{fill_price:,.2f} (mid=₹{mid_price:,.2f}, slippage=₹{slippage:,.2f})"
        )
        return {"filled": True, "price": fill_price, "slippage": round(slippage, 2),
                "order_id": order_id, "fallback_market": True}
